...
fn test_disk_space() -> Result<String> {
    ...
    if stats.blocks_available() * stats.fragment_size() < (10 * 1024 * 1024 * 1024) {
        // 10 GB
        Err(anyhow!("Disk space insufficient. Need at least 10 GB of free space to do something meaningful (You have {}).", HumanBytes(stats.blocks_available()*stats.fragment_size())))
    } else {
        Ok(format!(
            "Disk space is sufficient ({} free of {}).",
            HumanBytes(stats.blocks_available() * stats.fragment_size()),
            HumanBytes(stats.blocks() * stats.fragment_size())
        ))
    }
}
    ...
    for test in TEST_CASES {
        match test() {
            Ok(msg) => {
                ...
                lines.push(format!("{} {}", style("✓").green(), style(msg).green()))
            }
            Err(err) => {
                has_error = true;
